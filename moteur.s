...
MOTEUR_INIT	
	...
	;;Config Modes PWM0 + mode GenA + mode GenB
		...
		str r0, [r6]	
		;; Initialisation du PWM pour le moteur
	;; #define PWM_PERIOD (ROM_SysCtlClockGet() / 16000)
	;; SysCtlClockGet = 0x0F42400h => période calculée = 0x3E8
	;; Division par 2 car moteur 6V sur alimentation 12V

		ldr	r6, =PWM0LOAD         ; Charger l'adresse de PWM0LOAD
		mov	r0, #0x1F4           ; Période (divisée par 2)
		str	r0, [r6]             ; Configurer la période

		ldr	r6, =PWM0CMPA        ; Charger l'adresse de PWM0CMPA
		mov	r0, r12             ; Charger la vitesse actuelle depuis R12
		str	r0, [r6]            ; Configurer le rapport cyclique avec la vitesse

		ldr	r6, =PWM0CMPB        ; Charger l'adresse de PWM0CMPB
		str	r0, [r6]            ; Copier le rapport cyclique dans PWM0CMPB (dépend de CMPA)
